...
class VideosInScrollViewFragment : Fragment(R.layout.fragment_videos_in_scroll_view) {
  ...
  override fun onViewCreated(
    ...
    engine.setUp(data = VideoUrls.HlsSample, tag = "Z").bind(binding.topVideo)
      ...
      listOf(
        ...
        .map { (tag, container) ->
          Triple(
            tag,
            container,
            engine.setUp(data = VideoUrls.HlsSample, tag = tag).bind(container)
          )
        }
