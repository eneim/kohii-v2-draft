...
class VideoWithInteractionInScrollViewFragment :
  ...
  override fun onViewCreated(
    ...
    /* viewModel.selectedVideoRequest.observe(viewLifecycleOwner) { request ->
      // Remove the Player dialog to address the issue when programmatic orientation change
      // happens: when we reset the Activity orientation back to the "base" one, for some reason
      // the Player dialog is shown again, even if the selected request is null.
